...
public class ItemSpawner : MonoBehaviour
{
    ...
    [Tooltip("Vertical offset applied to spawned items.")]
    ...
    private void SpawnItems()
    {
        ...
        {
            Debug.LogWarning("No Item Database assigned.");
            return;
        }
    ...
    private Dictionary<Transform, int> CalculateRoomDistribution(int totalAmount)
    {
        ...
        // Distribute leftover items while there are still available spawn points.
    ...
    private void SpawnItem(Transform spawnPoint)
    {
        ...
        InventoryItemData item = itemDatabase.GetRandomItemByRarity(rarity);
        ...
        GameObject prefab = itemDatabase.GetPrefab(rarity);

        if (prefab == null)
        {
            Debug.LogWarning($"No prefab assigned for rarity: {rarity}");
            return;
        }
        ...
        GameObject spawnedItem = Instantiate(prefab, spawnPosition, Quaternion.Euler(spawnRotation), spawnedItemParent);

        ItemController itemController = spawnedItem.GetComponent<ItemController>();

        if (itemController == null)
        {
            Debug.LogWarning($"Prefab '{prefab.name}' does not have an ItemController component.");

            Destroy(spawnedItem);
            return;
        }

        itemController.referenceItem = item;
    ...
    private ItemRarity GetRandomRarity()
    {
        ...
        if (roll < commonSpawnChance)
            return ItemRarity.Bronze;

        if (roll < commonSpawnChance + uncommonSpawnChance)
